        ...
        <tr>
          ...
          <th v-if="showCheckboxes" class="w-12 px-4 py-3">
            <input
              ...
              class="rounded border-input text-accent"
      ...
      <tbody class="bg-card divide-y divide-border">
        ...
        <template v-if="loading">
          <tr v-for="i in 5" :key="'skeleton-' + i" class="animate-pulse">
            ...
            <td class="px-4 py-3"><div class="h-4 w-24 bg-muted rounded" /></td>
            <td class="px-4 py-3"><div class="h-4 w-16 bg-muted rounded" /></td>
        ...
        <tr v-else-if="contacts.length === 0">
          <td :colspan="showCheckboxes ? 7 : 6" class="px-4 py-12 text-center text-sm text-muted-foreground">
            No contacts found
          </td>
        </tr>
        ...
        >
          <!-- Checkbox -->
          <td v-if="showCheckboxes" class="w-12 px-4 py-3">
            <input
              type="checkbox"
          ...
          <td class="px-4 py-3 whitespace-nowrap text-sm text-muted-foreground">
            ...
            <span
              ...
              class="ml-1.5 inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium bg-accent/10 text-accent"
            >
          ...
          <td class="px-4 py-3 whitespace-nowrap text-sm text-muted-foreground">
            {{ contact.org }}
            <span v-if="contact.org && contact.title" class="text-border mx-1">&middot;</span>
          ...
          <td class="px-4 py-3 text-sm text-muted-foreground">
            <span
              ...
              :key="cat.id"
              class="inline-flex items-center mr-1 px-2 py-0.5 rounded-full text-xs bg-muted text-muted-foreground"
            >
              {{ cat.value }}
            </span>
            <span
          ...
          <td class="px-4 py-3 whitespace-nowrap text-right text-sm">
            <button
              class="text-destructive hover:text-destructive/80 ml-3"

...
function defaultStep(): StepFormItem {
  return { source_type: 'internal', dest_type: 'carddav', conflict_mode: 'auto', direction: 'pull', _src: emptyCardDAV(), _dst: emptyCardDAV() }
}
...
onMounted(async () => {
  try {
    const { data } = await listCredentials({ type: 'carddav' })

      ...
      <div v-else class="space-y-3">
        ...
        >
          <div class="min-w-0">
            ...
            <p v-if="p.last_error" class="text-xs text-red-500 dark:text-red-400 mt-0.5">
              {{ p.last_error }}
            </p>
          </div>
    ...
    <AppCard>
      ...
      <form @submit.prevent="handleConnect" class="space-y-3">
        ...
        <div class="flex items-center gap-3 pt-1">
          ...
          <span v-if="connectSuccess" class="text-sm text-green-600 dark:text-green-400"
...
async function handleConnect() {
  ...
  connectError.value = ''
  try {
    await connectCardDAV({
      url: form.value.url,
      username: form.value.username,
      password: form.value.password,
      skip_tls_verify: form.value.skipTLS,
    })

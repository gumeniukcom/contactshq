  ...
  <div class="max-w-3xl">
    ...
    <div v-else class="space-y-3">
      <div
        ...
        :key="c.id"
        class="bg-card rounded-lg border border-border p-4 flex items-start justify-between gap-4"
      >
        <div class="min-w-0 flex-1">
          ...
          <p class="text-sm text-foreground font-mono truncate">{{ c.remote_id }}</p>
          <p class="text-xs text-muted-foreground mt-1">{{ formatDateTime(c.created_at) }}</p>
          <div v-if="parsedDiffs(c).length > 0" class="mt-2 flex flex-wrap gap-1">
            <span
              v-for="d in parsedDiffs(c)"
              :key="d.field"
              class="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-amber-50 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-500/30"
            >
              {{ d.field }}
            </span>
          </div>
        ...
        <div class="flex flex-col gap-2 shrink-0">
          <RouterLink
            ...
            class="text-xs font-medium text-accent hover:text-accent/80 whitespace-nowrap"
          >
            Resolve →
          </RouterLink>
          <button
...
function statusClass(status: string) {
  if (status === 'pending') return 'bg-yellow-100 dark:bg-yellow-500/20 text-yellow-800 dark:text-yellow-300'

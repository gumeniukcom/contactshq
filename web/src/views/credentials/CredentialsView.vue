<template>
  <div class="max-w-3xl space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold text-foreground">Credentials</h1>
      <AppButton @click="openAdd">+ Add Credential</AppButton>
    </div>

    <!-- List -->
    <AppCard>
      <div v-if="loading" class="text-sm text-muted-foreground py-4 text-center">Loading...</div>

      <div v-else-if="credentials.length === 0" class="text-sm text-muted-foreground py-6 text-center">
      ...
      <table v-else class="w-full text-sm">
        ...
        <tbody class="divide-y divide-border">
          <tr v-for="cred in credentials" :key="cred.id" class="hover:bg-muted/50">
            <td class="py-3 pr-4 font-medium text-foreground">{{ cred.name }}</td>
            <td class="py-3 pr-4">

  ...
  <div class="max-w-3xl">
    ...
    <AppCard class="mb-6">
      ...
      <div class="flex items-center gap-2">
        <code
          class="flex-1 px-3 py-2 bg-muted border border-border rounded font-mono text-sm text-foreground break-all select-all"
          >{{ davUrl }}</code
        >
      ...
      <p class="mt-2 text-sm text-muted-foreground">
        Use an
        <RouterLink to="/settings/app-passwords" class="text-accent underline">App Password</RouterLink>
        instead of your main password for CardDAV clients.
      </p>
    </AppCard>
    ...
    <AppCard class="mb-6">
      ...
      <div class="space-y-4">
        ...
        <div class="p-4 bg-blue-50 dark:bg-blue-500/20 rounded-md">
          <p class="font-medium text-blue-900 dark:text-blue-300 mb-2">
            Option A: One-Tap Setup (Recommended)
          </p>
          <p class="text-sm text-blue-800 dark:text-blue-400 mb-3">
            Download a configuration profile that sets up CardDAV automatically.
          </p>
          <AppButton variant="primary" size="sm" @click="downloadProfile"> Download Profile </AppButton>
        </div>
          ...
          <ol class="list-decimal list-inside text-sm text-muted-foreground space-y-1">
            ...
            <li>
              ...
              <ul class="ml-6 mt-1 space-y-1 list-disc">
                <li>
                  Server: <code class="bg-muted px-1 rounded text-foreground">{{ hostname }}</code>
                </li>
                <li>
                  User Name: <code class="bg-muted px-1 rounded text-foreground">{{ userEmail }}</code>
                </li>
                <li>
                  Password: your
                  <RouterLink to="/settings/app-passwords" class="text-accent underline"
                    >app password</RouterLink
                  >
                </li>
              </ul>
            </li>
    ...
    <AppCard class="mb-6">
      ...
      <ol class="list-decimal list-inside text-sm text-muted-foreground space-y-1">
        ...
        <li>
          Enter URL: <code class="bg-muted px-1 rounded text-foreground break-all">{{ davUrl }}</code>
        </li>
        <li>
...
function downloadProfile() {
  ...
  fetch('/api/v1/setup/ios-profile', {
    ...
    .catch((err: unknown) => toast.error(getApiError(err, 'Failed to download profile')))
}
</script>
